...
async def ensure_channel() -> int:
    ...
    if channel_id:
        ...
        try:
            ...
            print(f"[{ts()}] Не удалось получить чат: {e}")
        return channel_id
...
async def cmd_snipe(client: Client, message: Message):
    ...
    if not channel_id:
        await message.reply(
            '<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Канал не найден. Добавь CHANNEL_ID в переменные окружения.'
        )
        return
...
async def cmd_stop(client: Client, message: Message):
    ...
    if not is_running or snipe_task is None:
        await message.reply(
            '<tg-emoji emoji-id="6028435952299413210">ℹ️</tg-emoji> Снайпинг не запущен.'
        )
        return
